...
// Toggle for development mode (true = bypass login timeout)
const devMode = true;
...
wss.on('connection', ws => {
    ...
    const authTimeout = setTimeout(() => {
        if (!ws.isAuthed) {
            if (devMode) {
                console.log(`Dev mode: allowing unauthenticated client from ${ws._socket.remoteAddress}`);
                ws.isAuthed = true;
            } else {
                console.log(`Client from ${ws._socket.remoteAddress} disconnected: no authentication`);
                ws.close();
            }
        }
    ...
    ws.on('message', message => {
        ...
        if (data.type === 'loadData') {
            ...
            if (playerData) {
                ...
                // Send them other players on the map
    ...
    ws.on('close', () => {
        ...
        if (username) {
            const player = connectedPlayers.get(username);
            if (player) {
                savePlayerData(username, player);
